...
global $CFG;
require_once($CFG->dirroot . '/question/type/truefalsegnrquiz/questiontype.php');
require_once($CFG->dirroot . '/question/engine/tests/helpers.php');
require_once($CFG->dirroot . '/question/type/edit_question_form.php');
require_once($CFG->dirroot . '/question/type/truefalsegnrquiz/edit_truefalsegnrquiz_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_truefalsegnrquiz_test extends advanced_testcase {
    protected $qtype;

    protected function setUp() {
        $this->qtype = new qtype_truefalsegnrquiz();
    }
    ...
    public function test_name() {
        $this->assertEquals($this->qtype->name(), 'truefalsegnrquiz');
    }
    ...
    public function test_get_possible_responses() {
        ...
        $this->assertEquals(array(
            $q->id => array(
                0 => new question_possible_response(get_string('false', 'qtype_truefalsegnrquiz'), 0),
                1 => new question_possible_response(get_string('true', 'qtype_truefalsegnrquiz'), 1),
    ...
    public function test_question_saving_true() {
        ...
        $questiondata = test_question_maker::get_question_data('truefalsegnrquiz');
        $formdata = test_question_maker::get_question_form_data('truefalsegnrquiz');

        $generator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $generator->create_question_category(array());

        $formdata->category = "{$cat->id},{$cat->contextid}";
        qtype_truefalsegnrquiz_edit_form::mock_submit((array)$formdata);

        $form = qtype_truefalsegnrquiz_test_helper::get_question_editing_form($cat, $questiondata);
